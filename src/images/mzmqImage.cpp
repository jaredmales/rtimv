...
constexpr size_t xrifDifferenceOffset =
    tv_nsecOffset + sizeof( uint64_t ); ///< The XRIF encoding differencing method.  Generally must be PIXEL.
...
int mzmqImage::imageKey( const std::string &sn )
{
    ...
    if( ats != std::string::npos )
    {
        ...
        {
            if( pcol > ats + 1 )
                m_server = sn.substr( ats + 1, pcol - ats - 1 );

            if( sn.size() > pcol + 1 )
                m_port = std::stoi( sn.substr( pcol + 1, sn.size() - pcol - 1 ) );
        }
        else
        {
            if( sn.size() > ats + 1 )
                m_server = sn.substr( ats + 1, sn.size() - ats - 1 );
        }
    }
    else
    {
...
void mzmqImage::imageThreadExec()
{
    ...
    while( !m_timeToDie )
    {
        ...
        while( !m_timeToDie && !reconnect ) // Inner loop waits for each new image and processes it as it comes in.
        {
            ...
            {
                if( m_timeToDie )
                    break; // This will true be if signaled during shutdown
            ...
            if( !m_imageAttached || m_timeToDie )
                continue; // Check that detach wasn't called.
...
std::vector<std::string> mzmqImage::info()
{
    ...
    info.push_back( std::string( imageName().size() + 1, ' ' ) + "mzmq://" + m_server + ":" +
