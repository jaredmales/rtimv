...
/** \todo consider making this an old-school virtual base class.  It would be a lot cleaner and simplify code in
 * rtimvMainWindow. However this will make it harder to make it shape agnostic.  Maybe there is a virtual interface
 * parent class?
 */
...
class StretchGraphicsItem
{
  ...
  public:
    ...
    float m_edgeTol{ 2 }; ///< Tolerance in pixels for selection, within this distance this object accepts the mouse.
    ...
    bool m_grabbing{
        false }; ///< Whether or not the cursor is in grabbing (open-hand) mode (true) or has grabbed (false).
...
void StretchGraphicsItem<QGraphicsItemT>::setCursorStatus( int cs )
{
    ...
    if( cs == cursorSizing )
    {
        ...
        if( m_sizing == szTopl )
            derived()->setCursor( QCursor( Qt::SizeFDiagCursor ) );
        if( m_sizing == szBotl )
            derived()->setCursor( QCursor( Qt::SizeBDiagCursor ) );
        if( m_sizing == szTopr )
            derived()->setCursor( QCursor( Qt::SizeBDiagCursor ) );
        if( m_sizing == szBotr )
            derived()->setCursor( QCursor( Qt::SizeFDiagCursor ) );
        if( m_sizing == szLeft )
            derived()->setCursor( QCursor( Qt::SizeHorCursor ) );
        if( m_sizing == szRight )
            derived()->setCursor( QCursor( Qt::SizeHorCursor ) );
        if( m_sizing == szTop )
            derived()->setCursor( QCursor( Qt::SizeVerCursor ) );
        if( m_sizing == szBot )
            derived()->setCursor( QCursor( Qt::SizeVerCursor ) );
        if( m_sizing == szAll )
            derived()->setCursor( QCursor( Qt::SizeAllCursor ) );
...
template <class QGraphicsItemT>
float StretchGraphicsItem<QGraphicsItemT>::edgeTol()
{
    return m_edgeTol;
}

template <class QGraphicsItemT>
void StretchGraphicsItem<QGraphicsItemT>::setEdgeTol( float et )
{
    m_edgeTol = et;
}
