...
StretchBox::StretchBox( qreal x, qreal y, qreal width, qreal height, QGraphicsItem *parent )
...
bool StretchBox::onHoverComputeSizing( QGraphicsSceneHoverEvent *e )
{
    ...
    else if( rx > rect().width() - m_edgeTol && rx < rect().width() + m_edgeTol && ry > rect().height() - m_edgeTol &&
             ry < rect().height() + m_edgeTol )
    {
        m_sizing = szBotr;
    }
    ...
    else if( rx > rect().width() - m_edgeTol && rx < rect().width() + m_edgeTol && ry > -m_edgeTol &&
             ry < rect().height() + m_edgeTol )
    {
        m_sizing = szRight;
    }
    ...
    else if( rx > -m_edgeTol && rx < rect().width() + m_edgeTol && ry > rect().height() - m_edgeTol &&
             ry < rect().height() + m_edgeTol )
    {
        m_sizing = szBot;
    }
...
void StretchBox::sizingCalcNewPos( QGraphicsSceneMouseEvent *e )
{
    ...
    {
        std::cerr << "StretchBox::sizingCalcNewPos (" << __FILE__ << " " << __LINE__
                  << "): m_sizing has invalid value.\n";
        return;
    }
